...
3. 核对 GitHub 已包含目标 `execution_commit`、服务器检出该提交且工作树干净；本机可以位于后续
   consumer 提交，但必须记录与 Stage 0 证据生成提交的兼容性判定。
...
14. S1.1 不消费 GPU 计算，只核验 Stage 0 G10 不可变索引、完整 Gate 集、交接资产身份和当前资格；这些输入有效时直接沿用，不重放 Stage 0。
15. 若 G10 的 `producer_commit` 与本次 `consumer_commit` 不同，必须对精确 Git 差异逐项分类，并发布独立、内容寻址的复用证明；未分类变化 fail-closed。
16. S1.1 只记录 CUDA/NCCL 的运行时可见性和 G10 交接引用，不为生成入口合同重新占用 GPU、建立租约或执行 collective smoke。
17. S1.7 真正消费单卡前，再对候选 GPU 执行最小 CUDA 上下文/分配检查，检查外部占用并取得对应的短期项目租约。
18. S1.8 真正消费多卡前，再取得资源所有者确认、核对每张候选卡、建立 UUID 绑定租约并执行当次最小 NCCL collective smoke；旧报告只能帮助选候选，不能替代消费边界检查。
19. 若 boot ID、GPU UUID/拓扑、驱动/运行时、资产资格或外部占用发生漂移，只重验受影响的最小边界；smoke 完成后立即释放本次租约，不扩大为 Stage 0 整链重跑。
...
- 专用分支、执行提交、消费提交、证据复用判定与按需同步记录。
- 缓存/tmp 解析、写路径审计和 GPU UUID 租约策略边界。
- G10 入口资产/环境交接摘要，以及 S1.7/S1.8 的最小健康重验触发点。
...
- 开发准备阶段允许保留已记录的用户修改；任何计入正式 Gate 的服务器运行，其执行代码、配置和数学契约必须已提交，GitHub 必须可达该 `execution_commit`，执行服务器必须检出该提交且工作树干净。其他端可以位于后续 consumer 提交，不要求长期同一 HEAD。
- `Agent/` 文件集合必须恰为规定的五份，且逐文件 SHA-256 一致。
- S1.1 的 `G1-ENTRY` 不声明当前 GPU 可用性，也不以重跑 CUDA/NCCL 为通过条件；S1.7/S1.8 必须分别在实际消费边界完成当次单卡/多卡最小健康检查。
- 跨提交沿用 G10 时，复用证明必须覆盖从生成提交到消费提交的全部变化、验证引用和保留 Gate；仅因 HEAD 改变不得触发 Stage 0 整链重跑。
...
- `G1-ENTRY` 只证明 S1.1 的入口、资产和契约边界成立；它不能替代 S1.7/S1.8 的实时 GPU/NCCL 资格检查。
