...
上面的 3.1–3.6 继续保留为计划编制时的历史快照；当前实施状态以本节和版本化报告为准：

- **G0-C：`PASS`**。SSH 身份与 `Agent/*.md` 已重验，两端五份文件哈希一致。
- **G0-G：`PASS`**。管理员路径 B 已持久化为精确四 UUID 排除/四 UUID 可用合同；
  管理员 finalizer、ECC/row-remap、逐卡 CUDA 和最小四卡 NCCL 通信均通过。
- **G0 总 gate：`PASS`**。
- **G1：`PASS`**。截至 2026-08-03，用户对仅 Stage 0 可再生 smoke 的单盘风险接受仍在
  有效期内；它在 2026-08-18 23:59 CST 或 Stage 4 开始前（先发生者）失效，不覆盖
  Stage 4/5 正式产物。
- **G2：`PASS`**。离线重建候选已完成 GPU 后验资格检查，并发布
  `training_eligible=true` 的原子推荐引用。
- 按用户要求，**S0.4–S0.12 暂停**；本次最小 NCCL smoke 不构成 S0.7/G6 完成。

机器可读结论见 `reports/stage0/g0-g-gpu-final-20260803.json` 和
...
4. NCCL 每个消息规模先执行 20 次 warmup，再测量 50 个样本；每个样本取连续 3 次
   ...
   256 KiB、1 MiB、16 MiB（`[65536, 262144, 4194304]` 个 float32 元素），协议版本
   `stage0-unified-measurement-v2`，并固定 `NCCL_P2P_DISABLE=1`；进程组重建三次。
   延迟报告中位数、p95 和重复间变异系数，吞吐使用相同样本计算。
