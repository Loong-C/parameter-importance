...
def _row(method: str, *, semantic: str, anchor: str, repetition: int = 0, wall: float = 10.0, gpu: str = APPROVED_GPU_UUIDS[0], io: bool = True) -> dict[str, object]:
    return {
        "method": method,
        "semantic": semantic,
        "run_id": f"s209-{semantic[:4]}",
...
def _costs(*, io: bool = True) -> dict[str, object]:
    scientific = [_row(method, semantic=S29_COST_SEMANTICS[0], anchor="shared", wall=10.0, io=io) for method in ("raw", "double", "u")]
    isolated = [_row(method, semantic=S29_COST_SEMANTICS[1], anchor="isolated", wall=11.0, io=io) for method in ("raw", "double", "u")]
    ...
    for repetition, anchor in enumerate(("online-a", "online-b")):
        ...
        for method, wall in (("double", 11.0), ("raw", 10.0), ("u", 12.0)):
            online.append(_row(method, semantic=S29_COST_SEMANTICS[2], anchor=anchor, repetition=repetition, wall=wall, io=io))
    return {
        S29_COST_SEMANTICS[0]: {"defined": True, "observations": scientific},
...
def test_s209_reducer_is_strict_and_idempotent() -> None:
    ...
    row = _row("raw", semantic=S29_COST_SEMANTICS[0], anchor="shared")
