...
当前没有获授权的第二故障域。`docs/stage0/persistence-decision.md` 保持 `PENDING_USER_DECISION`，直至以下两条路径之一成立：

1. 用户授权异盘/异主机目标、访问边界、加密和保留策略，随后完成 manifest 驱动的备份与受控恢复演练；
2. 用户明确接受列定范围、损失、有效期与终止条件的单盘风险。

该决策未完成时 G1-D 与 G1 均不通过；Stage 4/5 前必须重新审查，即使 Stage 0 曾有有效风险接受。
