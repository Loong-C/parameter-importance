...
## 2026-08-03 14:24 CST — 封存服务器未跟踪 GPU 恢复文件

### 目标与范围

- 在同步现行 Stage 0 分支前，把服务器工作树中六个未跟踪的历史 GPU 恢复文件形成可复核提交并回传本机，避免快进同步覆盖或丢失服务器已有内容。
- 本阶段只封存已有文件，不执行管理员脚本、不修改 GPU/服务/驱动、不清除 ECC、不删除或改名任何服务器文件。

### 审查结果

- 六个文件均位于 Git 仓库允许路径；两份授权 JSON 与现行本机版本 SHA-256 一致。
- `admin_install_gpu_uuid_exclusion_and_reboot.sh` 与现行本机版本 SHA-256 一致。
- 另外三个脚本是现行本机版本的较早实现：服务器旧版缺少后续已验证的安全恢复、UUID 规范化或诊断记录改进；差异已逐行审查，不把旧版重新选为现行实现。
- 服务器旧版仍保留在本次提交历史中；后续从本机合并时保留现行已验证版本作为工作树内容。

### 验证与同步状态

- 服务器分支/起点：`feat/stage0-infrastructure` / `5cc53930a3f745fbd3e9ea4e171bd0773172984a`。
- 本机现行起点：`feat/stage0-completion` / `11a3f9072b01ab122d3bd3dc947f637dcfe8d755`。
- 两端差异和逐文件 SHA-256 已由本机保存于 Git 忽略的临时审计目录；不把临时副本作为正式实现。
- 下一步：在服务器静态检查并提交六个历史文件和本日志条目；用 Git bundle 回传本机，显式合并并复验，随后从本机统一推送和同步。
