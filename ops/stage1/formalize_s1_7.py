...
def _validate_fixture_manifest(value: object) -> dict[str, Any]:
    ...
    if model_identity.get("parameter_count") != 14067712 or tokenizer_identity.get("vocab_size") != 50277:
    ...
    if not dropout or any(value != 0.0 for value in dropout.values()):
...
def canonical(value): return hashlib.sha256(json.dumps(value,sort_keys=True,separators=(',',':'),ensure_ascii=True).encode('utf-8')).hexdigest()
def sha_file(path):
 digest=hashlib.sha256()
 with path.open('rb') as handle:
