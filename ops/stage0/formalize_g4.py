#!/usr/bin/env python3
"""Execute the canonical formal Stage 0 G4 configuration/provenance gate."""
...
from param_importance_nlp.stage0_g4 import execute_stage0_g4
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--g3-index-ref", required=True)
    ...
    if configured is None or Path(configured).resolve(strict=True) != data_root:
        raise RuntimeError("STAGE0_G4_DATA_ROOT_ENV_MISMATCH")
    binding = inspect_stage0_source(
        arguments.repository,
        expected_commit=arguments.generator_git_commit,
    )
    result = execute_stage0_g4(
        binding=binding,
        data_root=data_root,
        g3_index_ref=arguments.g3_index_ref,
    )
    print(
        json.dumps(
            {
