#!/usr/bin/env python3
"""Execute the formal Stage 0 G9 layered-test and replay gate."""
...
from param_importance_nlp.stage0_g9 import execute_stage0_g9


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--data-root", type=Path, required=True)
    parser.add_argument("--g8-index-ref", required=True)
    parser.add_argument("--repository", type=Path, required=True)
    parser.add_argument("--git-commit", required=True)
    parser.add_argument("--git-branch", required=True)
    arguments = parser.parse_args(argv)
    result = execute_stage0_g9(
        ...
        g8_index_ref=arguments.g8_index_ref,
    )
    print(
        json.dumps(
            {
