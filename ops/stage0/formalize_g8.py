#!/usr/bin/env python3
"""Execute the canonical Stage 0 S0.10 capacity and operations suite."""
...
from param_importance_nlp.stage0_g8 import execute_stage0_g8
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--g7-recovery-index-ref", required=True)
    ...
    if configured is None or Path(configured).resolve(strict=True) != data_root:
        raise RuntimeError("STAGE0_G8_DATA_ROOT_ENV_MISMATCH")
    binding = inspect_stage0_source(
        arguments.repository,
        expected_commit=arguments.generator_git_commit,
    )
    result = execute_stage0_g8(
        binding=binding,
        data_root=data_root,
        g7_recovery_index_ref=arguments.g7_recovery_index_ref,
    )
    print(
        json.dumps(
            {
